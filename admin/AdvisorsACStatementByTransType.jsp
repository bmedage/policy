...
<table border = "1" id="table" style = "width : 100%;margin:0 auto;">
	 					...
	 					<tr style = "height:40px;">
	  						...
	  						<th><center><b><%
	  						int name5 = 0;
	  						if(request.getParameter("transType").equals("All"))
	  						{
	  						...
	  						{
	  						PreparedStatement pstmt5=con.prepareStatement("SELECT SUM(Admin) FROM transaction where pinNo=? And tranType=? And year=?");
	  						pstmt5.setString(1, request.getParameter("advCode"));
						...
						<%=name5 %>	
	  						...
	  						if(request.getParameter("transType").equals("All"))
	  						{
	  						PreparedStatement pstmt6=con.prepareStatement("SELECT SUM(TDS) FROM transaction where pinNo=? And tranType=? And year=?");
	  						...
	  						{
	  							PreparedStatement pstmt6=con.prepareStatement("SELECT SUM(TDS) FROM transaction where pinNo=? And year=?");
		  						pstmt6.setString(1, request.getParameter("advCode"));
		  						pstmt6.setString(2, request.getParameter("year"));
		  						ResultSet res6 = pstmt6.executeQuery();
		  						while (res6.next()) 
		  						{
		  							name6 = res6.getInt(1);
						...
						<%=name6 %>	
	  						...
	  						{
	  							PreparedStatement pstmt1=con.prepareStatement("SELECT SUM(debit) FROM transaction where pinNo=? And tranType=? And year=?");
		  						pstmt1.setString(1, request.getParameter("advCode"));
						...
						<%=name %>	
	  						...
	  						{
	  							PreparedStatement pstmt2=con.prepareStatement("SELECT SUM(credit) FROM transaction where pinNo=? And tranType=? And year=?");
		  						pstmt2.setString(1, request.getParameter("advCode"));
						...
						<%=name1 %>	
	  						...
	  						{
	  							PreparedStatement pstmt3=con.prepareStatement("SELECT SUM(balance) FROM transaction where pinNo=? And tranType=? And year=?");
		  						pstmt3.setString(1, request.getParameter("advCode"));
						...
						<tr style = "height:40px;">
	 					...
	 					{
	 						PreparedStatement pstm=con.prepareStatement("select * from transaction where pinNo=? And tranType=? And year=?");
		 					pstm.setString(1, request.getParameter("advCode"));
		 					pstm.setString(2, request.getParameter("transType"));
		 					pstm.setString(3, request.getParameter("year"));
		 					ResultSet rs1=pstm.executeQuery();
		 					int count=0;
		 					while(rs1.next())
		 					{
